...
export default class Upload extends Component {
    ...
    componentDidMount() {
        ...
        let Uploader = Qiniu.uploader({
            ...
            uptoken_url: '/dj-api/private/qiniu/genQiNiuToken',         // Ajax请求uptoken的Url，强烈建议设置（服务端提供）
...
//                var sourceLink = domain + "/" + res.key; //获取上传成功后的文件的Url
    ...
    render() {
        let props = this.props;
        return (
            <div className={`upload_${props.type}`} id={props.container}>
                {
                    props.type === 'avatar' ? (
                        <div className="upload_avatar_imgBox">
                            {this.state.imgSrc!=''?( <img src={this.state.imgSrc} alt=""/>):''}
                            <a href="javascript:;" className="upload_avatar_btn" id={props.id}></a>
                        </div>

                    ) : (
                        <div className={`upload_${props.type}_imgBox`}>
                            {this.state.imgSrc!=''?( <img src={this.state.imgSrc} alt=""/>):''}

                            <a href="javascript:;" className={`upload_${props.type}_btn`} id={props.id}>{props.content}</a>
                        </div>
                    )
                }
            </div>
